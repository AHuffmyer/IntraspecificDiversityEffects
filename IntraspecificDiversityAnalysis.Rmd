...
```{r, restults=FALSE, warning=FALSE, message=FALSE}
juvtemps<-read.csv("Data/Juvenile_ApexTemps.csv", header=T, na.strings="NA")
...
ggsave(filename="Figures/Juvenile_timeseries_temp.png", plot=TimeSeries1j, dpi=500, width=16, height=8, units="in")
```
...
ggsave(filename="Figures/Juvenile_24h_temp.png", plot=JuvTempsDay, dpi=500, width=8, height=8, units="in")
```


## 2. Selection and Complementarity Calculations  
...
#subset fused corals to obtain fused with self and fused with non-self
fused_end<-end[which(end$Fusion.Type == "Fused"),]
