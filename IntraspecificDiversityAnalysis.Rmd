...
```{r}
Ohdepth<-read_tsv("Data/SOD.counts")%>%select(-11)%>%mutate(mean=rowMeans(.[,1:10]))
...
rawdat<-read_tsv("Data/SOD.geno",col_names=FALSE)%>%select(-33)%>%unite(marker,X1,X2,sep="_")
colnames(rawdat)[2:31]<-as.character(names$x)

names<-as.data.frame(cols[2:31])%>%rename(list=1)%>%separate(list,into=c("list","trash"))%>%select(-trash)%>%mutate(n=rep(c(0,1,2),10))%>%unite(x,list,n,sep="_")
