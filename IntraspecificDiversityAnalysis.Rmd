...
author: "Ariana Huffmyer, Crawford Drury"
date: "2021"
output:
  html_document:
    code_folding: hide
...
## Generate full larval figure  

```{r}
larvalfig<-plot_grid(larv_surv_plot, larv_settle_plot, ncol=1, nrow=2, rel_heights= c(1,1), rel_widths = c(1,1), align="vh")
...
glmer(success~Temperature * Treatment * Genotype + (1|Plate), data=endstress, family="binomial") 
Anova(model_surv, type=2)
``` 
...
model_growth<-lmer(TotalPercentDay~Temperature * Treatment * Genotype + (1|Plate), data=bwt_analysis)
qqPlot(residuals(model_growth))
leveneTest(TotalPercentDay~Temperature * Treatment * Genotype, data=bwt_analysis)
...
model_pam<-lmer(Change~Temperature * Treatment * Genotype + (1|Plate), data=pam_analysis)
qqPlot(residuals(model_pam))
leveneTest(Change~Temperature * Treatment * Genotype, data=pam_analysis)
...
```{r}
adultfig<-plot_grid(surv_plot, bwt_plot, pam_plot, ncol=1, nrow=3, rel_heights= c(1,1,1), rel_widths = c(1,1,1), align="vh")
...
environ_panel<-plot_grid(growtemp_plot, temp_plot, growlight_plot,light_plot, ncol=2, nrow=2, rel_heights= c(1,1, 1, 1), rel_widths = c(1,1, 1, 1), label_size = 10, label_y=1, align="vh")
