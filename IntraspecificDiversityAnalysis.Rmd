...
ddply(finalsurv, c("diversity_level", "Temperature"), summarise,
...
#generate sample size
ddply(finalsettle, c("diversity_level", "Temperature"), summarise,
                            N    = length(outcome[!is.na(outcome)]))
...
In the juvenile experiment, we exposed settled corals to single or multiple genotype groups on glass slides by securing sections of waterproof paper with respective juveniles. All data in this experiment was recorded at the level of the individual juvenile. Juveniles were exposed to either ambient or high temperature for 32 days in controlled indoor tanks with aquarium lighting and supplied with filtered seawater. Light intensity peaked daily at ~250 PAR in this indoor tank system.  

## Selection and Complementarity Calculations  

### Survival   

Load data and subset the end timepoint for calculations of survival and subset at a midpoint for measurements of growth (this subset is due to high mortality in the second half of the treatment period). Correct parent colony ID with clonal ID. Juveniles were exposed to groups of 2, 3, or 4 other juveniles. As diversity calculations are not changed when N and Q are equal, we are using a N and Q value of 4 for these calculations.  
...
``` 
Display sample sizes and calculate values as described above in the Larval Experiment section.    
...
#generate sample size
ddply(end, c("Richness.Type", "Treatment"), summarise,
...
Calculate diversity values for Survival.  
```{r, results=FALSE}
#calculate coefficients
...
Growth will be analyzed at a midpoint through the stress test, as there were too few individuals alive to measure growth at the end. Growth is measured as polyp expansion (total number of polyps at the end) as all juveniles started with one polyp.    

Calculate values for growth and display sample sizes.  
...
#generate sample size
ddply(growth_mid, c("Richness.Type", "Treatment"), summarise,
...
View juvenile diversity values.  
```{r, results=TRUE}
...
Diversity metrics are calculated as described above.  
...
#generate sample size
ddply(endstress, c("Treatment", "Temperature"), summarise,
...
#generate sample size
ddply(bwt_stress, c("Treatment", "Temperature"), summarise,
...
#generate sample size
ddply(final, c("Treatment", "Temperature"), summarise,
...
Calculate the proportion fthat complementarity values contribute to net effect.  
