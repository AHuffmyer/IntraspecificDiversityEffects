...
Plot larval survivorship by genotype.  
```{r}
...
larv_surv_plot<-ggplot(data=larv_surv_summary, aes(x = Temperature, y = mean, color = Treatment, shape=Treatment, linetype=Treatment, group=interaction(Treatment, Temperature))) +
  ...
  scale_shape_manual(name="Diversity Treatment", values=c(1, 19))+
  scale_linetype_manual(name="Diversity Treatment", values=c("dotted", "solid"))+
...
Plot larval survivorship by treatment.  
```{r}
larv_surv_summary_trt<-selection_surv%>%
  gather(key="Treatment", Diverse:Single, value="surv")%>%
  group_by(Treatment, Temperature)%>%
  summarise(mean=mean(surv), sd=sd(surv), N=length(surv), se=sd/sqrt(N))%>%
  mutate(code=paste(Treatment, Temperature))

larv_surv_plot_trt<-ggplot(data=larv_surv_summary_trt, aes(x = Temperature, y = mean, color = Treatment, shape=Treatment, linetype=Treatment, group=interaction(Treatment, Temperature))) +
    geom_point(size=3, position=position_dodge(0.1), color="black") +
   geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, linetype="solid", position=position_dodge(0.1), size=1.3, color="black") +
  geom_line(position=position_dodge(0.1), size=1.3, aes(group=Treatment), color="black") +
  scale_shape_manual(name="Diversity Treatment", values=c(1, 19))+
  scale_linetype_manual(name="Diversity Treatment", values=c("dotted", "solid"))+
    xlab("Temperature Treatment") + 
    ylim(0, 1)+
  geom_text(x=1.5, y=0.5, label=expression(italic("Temperature p<0.001")), size=4, color="black") +
    geom_text(x=1.5, y=0.45, label=expression(italic("Diversity p=0.006")), size=4, color="black") +
    geom_text(x=1.5, y=0.4, label=expression(italic("Interaction p=0.007")), size=4, color="black") +
    ylab(expression(bold(paste("Survival"))))+
    theme_classic() + 
    theme(
      legend.title=element_text(face="bold", size=14),
...
Plot larval settlement by genotype.  
```{r}
...
Plot larval settlement by treatment.  
```{r}
larv_settle_summary_trt<-selection_settle%>%
  gather(key="Treatment", Diverse:Single, value="settle")%>%
  group_by(Treatment, Temperature)%>%
  summarise(mean=mean(settle), sd=sd(settle), N=length(settle), se=sd/sqrt(N))%>%
  mutate(code=paste(Treatment, Temperature))
...
selection_settle%>%
  ...
  group_by(Treatment)%>%
  summarise(mean=mean(settle), sd=sd(settle), N=length(settle), se=sd/sqrt(N))%>%
  mutate(code=paste(Treatment))

larv_settle_plot_trt<-ggplot(data=larv_settle_summary_trt, aes(x = Temperature, y = mean, color = Treatment, shape=Treatment, linetype=Treatment, group=interaction(Treatment, Temperature))) +
    geom_point(size=3, position=position_dodge(0.1), color="black") +
   geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, linetype="solid", position=position_dodge(0.1), size=1.3, color="black") +
  geom_line(position=position_dodge(0.1), size=1.3, aes(group=Treatment), color="black") +
  scale_shape_manual(name="Diversity Treatment", values=c(1, 19))+
  scale_linetype_manual(name="Diversity Treatment", values=c("dotted", "solid"))+
    geom_text(x=1.5, y=0.75, label=expression(italic("Temperature p=0.960")), size=4, color="darkgray") +
    geom_text(x=1.5, y=0.72, label=expression(italic("Diversity p=0.022")), size=4, color="black") +
    geom_text(x=1.5, y=0.69, label=expression(italic("Interaction p=0.961")), size=4, color="darkgray") +
    xlab("Temperature Treatment") + 
    ylim(0.5, 1)+
    ylab(expression(bold(paste("Settlement Rate"))))+
    theme_classic() + 
...
Analyze model for larval survival using tempreature and treatment as main effects. We are also going to add genotype as a random effect to account for repeated measures. We are also including plate as a random effect due to the experiment being repeated each day.    
```{r}
model_larv_surv1<-glmer(success~Temperature * Treatment + (1|Plate) + (1|Genotype), data=finalsurv, family="binomial") 
summary(model_larv_surv1)
Anova(model_larv_surv1, type=2) 
dispersion_glmer(model_larv_surv1) 
```

### Settlement  

Analyze model for settlement success using temperature and treatment as main effects. We are also going to add genotype as a random effect to account for repeated measures. We are also including plate as a random effect due to the experiment being repeated each day.   
```{r}
model_settle_1<-glmer(success~Temperature * Treatment + (1|Plate) + (1|Genotype), data=finalsettle, family="binomial")
summary(model_settle_1)
Anova(model_settle_1, type=2)
dispersion_glmer(model_settle_1) 
```

## 5. Generate full larval figure  

```{r}
larvalfig_geno<-plot_grid(larv_surv_plot, larv_settle_plot, labels=c("A", "B"), label_size=22, ncol=1, nrow=2, rel_heights= c(1,1), rel_widths = c(1,1), align="vh")

ggsave(filename="Figures/Larval_Figure_Geno.png", plot=larvalfig_geno, dpi=500, width=12, height=8, units="in")

larval_surv_plot_trt2<-larv_surv_plot_trt+theme(legend.position="none")

larvalfig_trt<-plot_grid(larval_surv_plot_trt2, larv_settle_plot_trt, labels=c("A", "B"), label_size=18, ncol=2, nrow=1, rel_heights= c(1,1), rel_widths = c(1,1.5), align="vh")

ggsave(filename="Figures/Larval_Figure_Trt.png", plot=larvalfig_trt, dpi=500, width=9, height=4, units="in")
```
...
Plot survivorship for each treatment.  
```{r}
surv_summary_trt<-endstress%>%
  group_by(Treatment, Temperature)%>%
  summarise(mean=mean(success), sd=sd(success), N=length(success), se=sd/sqrt(N))%>%
  mutate(code=paste(Treatment, Temperature))

surv_plot_trt<-ggplot(data=surv_summary_trt, aes(x = Temperature, y = mean, color = Treatment, shape=Treatment, linetype=Treatment, group=interaction(Treatment, Temperature))) +
    geom_point(size=3, position=position_dodge(0.1), color="black") +
   geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, linetype="solid", position=position_dodge(0.1), size=1.3, color="black") +
  geom_line(position=position_dodge(0.1), size=1.3, aes(group=Treatment), color="black") +
  scale_shape_manual(name="Diversity Treatment", values=c(1, 19))+
  scale_linetype_manual(name="Diversity Treatment", values=c("dotted", "solid"))+
    geom_text(x=1.5, y=0.5, label=expression(italic("Temperature p<0.001")), size=4, color="black") +
    geom_text(x=1.5, y=0.45, label=expression(italic("Diversity p<0.001")), size=4, color="black") +
    geom_text(x=1.5, y=0.4, label=expression(italic("Interaction p=0.028")), size=4, color="black") +
  ylim(0,1)+
    xlab("Temperature Treatment") + 
    ylab(expression(bold(paste("Survival"))))+
    theme_classic() + 
...
Analyze total survival using a binomial model with success/failure vectors which will allow us to incorporate random effects. Treatment and temperature are main effects and genotype is included as a random intercept due to repeated measures on multiple fragments of the same genotype.    
...
model_surv_a<-glmer(y~Temperature * Treatment + (1|Genotype), family=binomial, data=endstress) 
...
```{r}
Anova(model_surv_a, type="II")
summary(model_surv_a)
dispersion_glmer(model_surv_a) 
```
...
ggsave("Figures/Colony_SOD.png", plot=sod_plot, height=6, width=8, units = c("in"), dpi=300) #output figure
```
...
Calcification (growth) was measured using the buoyant weight technique. We are analyzing growth as total percent growth (grams per gram per day) from the beginning to the end of the stress test period.  

Load in buoyant data from Excel worksheet.    
...
Plot bwt in each treatment.  
```{r}
bwt_summary_trt<-bwt_stress%>%
  group_by(Treatment, Temperature)%>%
  filter(!TotalPercentDay> 1.1)%>%
  summarise(mean=mean(TotalPercentDay, na.rm=TRUE), sd=sd(TotalPercentDay, na.rm=TRUE), N=length(TotalPercentDay[!is.na(TotalPercentDay)]), se=sd/sqrt(N))%>%
  mutate(code=paste(Treatment, Temperature))

bwt_plot_trt<-ggplot(data=bwt_summary_trt, aes(x = Temperature, y = mean, color = Treatment, shape=Treatment, linetype=Treatment, group=interaction(Treatment, Temperature))) +
    geom_point(size=3, position=position_dodge(0.1), color="black") +
   geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, linetype="solid", position=position_dodge(0.1), size=1.3, color="black") +
  geom_line(position=position_dodge(0.1), size=1.3, aes(group=Treatment), color="black") +
  scale_shape_manual(name="Diversity Treatment", values=c(1, 19))+
  scale_linetype_manual(name="Diversity Treatment", values=c("dotted", "solid"))+
    geom_text(x=1.5, y=0.22, label=expression(italic("Temperature p<0.001")), size=4, color="black") +
    geom_text(x=1.5, y=0.205, label=expression(italic("Diversity p=0.880")), size=4, color="darkgray") +
    geom_text(x=1.5, y=0.19, label=expression(italic("Interaction p=0.502")), size=4, color="darkgray") +
    ylim(0,0.25)+
    xlab("Temperature Treatment") + 
    ylab(expression(bold(paste("% growth day"^-1))))+
    theme_classic() + 
...
Conduct linear mixed effect model to analyze the effect of temperature treatment and diversity as main effects with genotype as a random intercept due to repeated measures of each genotype.  
...
model_growth_trt<-lmer(TotalPercentDay~Temperature * Treatment + (1|Genotype), data=bwt_analysis)
qqPlot(residuals(model_growth_trt))
anova(model_growth_trt, type=2)
summary(model_growth_trt)

rand(model_growth_trt)

leveneTest(residuals(model_growth_trt)~Temperature*Treatment, data=bwt_analysis)
```

## 4. PAM    

Photosynthetic efficiency (measured by PAM method). This value is collected as relative change in maximum quantum yield (Fv/Fm) for each fragment from the beginning to the end of the stress test.  

Load in PAM data sets and match the record number to sample.     
...
Plot pam data for each genotype in each treatment.  
```{r}
...
Plot pam data for each treatment.  
```{r}
pam_summary_trt<-totalPAM%>%
  group_by(Treatment, Temperature)%>%
  summarise(mean=mean(Change, na.rm=TRUE), sd=sd(Change, na.rm=TRUE), N=length(Change[!is.na(Change)]), se=sd/sqrt(N))%>%
  mutate(code=paste(Treatment, Temperature))

pam_plot_trt<-ggplot(data=pam_summary_trt, aes(x = Temperature, y = mean, color = Treatment, shape=Treatment, linetype=Treatment, group=interaction(Treatment, Temperature))) +
    geom_point(size=3, position=position_dodge(0.1), color="black") +
   geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, linetype="solid", position=position_dodge(0.1), size=1.3, color="black") +
  geom_line(position=position_dodge(0.1), size=1.3, aes(group=Treatment), color="black") +
  scale_shape_manual(name="Diversity Treatment", values=c(1, 19))+
  scale_linetype_manual(name="Diversity Treatment", values=c("dotted", "solid"))+
    geom_text(x=1.5, y=-0.2, label=expression(italic("Temperature p<0.001")), size=4, color="black") +
    geom_text(x=1.5, y=-0.23, label=expression(italic("Diversity p=0.812")), size=4, color="darkgray") +
    geom_text(x=1.5, y=-0.26, label=expression(italic("Interaction p=0.446")), size=4, color="darkgray") +
    xlab("Temperature Treatment") +
    ylim(-0.3, 0.2)+
    ylab(expression(bold(paste("% change in Fv/Fm"))))+
    theme_classic() + 
...
Conduct linear mixed effect model to analyze the effect of temperature treatment and diversity as main effects on growth. We will include genotype as a random intercept to account for genotype repeated measures.  
 
```{r}
pam_analysis<-totalPAM

model_pam_trt<-lmer(Change~Temperature * Treatment + (1|Genotype), data=pam_analysis)
qqPlot(residuals(model_pam_trt))
anova(model_pam_trt, type=2)
summary(model_pam_trt)

rand(model_pam_trt)

leveneTest(Change~Temperature*Treatment, data=pam_analysis)
``` 

## Combine all adult figures  

```{r}
adultfig_geno<-plot_grid(surv_plot, bwt_plot, pam_plot, labels=c("A", "B", "C"), label_size=22,  label_fontface = "bold", ncol=1, nrow=3, rel_heights= c(1,1,1), rel_widths = c(1,1,1), align="vh")

ggsave(filename="Figures/Adult_Figure_Geno.png", plot=adultfig_geno, dpi=500, width=12, height=10, units="in")


surv_plot_trt2<-surv_plot_trt+theme(legend.position="none")
bwt_plot_trt2<-bwt_plot_trt+theme(legend.position="none")

adultfig_trt<-plot_grid(surv_plot_trt2, bwt_plot_trt2, pam_plot_trt, labels=c("A", "B", "C"), label_size=18,  label_fontface = "bold", ncol=3, nrow=1, rel_heights= c(1,1,1), rel_widths = c(1,1,1.5), align="vh")

ggsave(filename="Figures/Adult_Figure_Trt.png", plot=adultfig_trt, dpi=500, width=12, height=4, units="in")
```
...
NOTE FOR FORD: THIS PART NOT WORKING. Reading in the beagle file generates col names with lots of "..." rather than underscores as required in the script. 
```{r}
Ohdepth<-read_tsv("Data/SOD.counts")%>%dplyr::select(-11)%>%mutate(mean=rowMeans(.[,1:10]))
loci<-read_tsv("Data/SOD.beagle")%>%dplyr::select(marker,allele1,allele2)
cols<-colnames(read_tsv("Data/SOD.beagle")%>%dplyr::select(marker,contains('Ind')))

rawdat<-read_tsv("Data/SOD.geno",col_names=FALSE)%>%dplyr::select(-X33)%>%unite(marker,X1,X2,sep="_")
...
b<-ggplot(activity)+geom_boxplot(aes(reorder(colony,mean),SOD,fill=genotype))+
     ...
     annotate("text",x=1,y=0.08,label="Wilcox ~Genotype p=0.092",size=2,fontface="italic",color="darkgray",hjust=0)
